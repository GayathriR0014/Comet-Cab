...
public class MakeBookingDAOImpl implements MakeBookingDAO {
	...
	{System.out.println("inside trip");
		String licenseInserted=licenseNo;
		try{
		int status=0;
		conn1 = db1.getConnection();
		
		// Test begins
		System.out.println("BEFORE SQL 2");
		System.out.println("BK ID :"+bookingId);
		System.out.println("DR ID :"+driverId);
		System.out.println("LN ID :"+licenseNo);
		// test ends
		
		
		ps1 =conn1.prepareStatement("Insert into trip(bookingId,driverId,licenseNo) values(?,?,?)");
		ps1.setString(1,String.valueOf(bookingId));
		ps1.setString(2,driverId);
		ps1.setString(3,licenseNo);
		status = ps1.executeUpdate();
		conn1.close();
		}catch(Exception e){
		System.out.println(e);
	}
		return (licenseInserted);
	}
	...
	public Driver RetrieveDriverDetails(String licenseNo,BigInteger bookingId)
	{
		...
		try{
			conn1 = db1.getConnection();
			ps1 =conn1.prepareStatement("Select firstname,lastname,phoneNo,driverId from driver where licenseNo= ?");
			...
			while(rs1.next()){
				...
				driverInfo.setDriverId(rs1.getString(4));
				}
			conn1.close();
			}catch(Exception e){
			System.out.println(e);
	...
	public Booking SetBookingInfo(Booking bookingInfo)
	{
			...
			status = ps1.executeUpdate();
			conn1.close();
			bookingInfo=GetBookingId(bookingInfo);
			}catch(Exception e){
			System.out.println(e);
		}
		return bookingInfo;
...
public Booking GetBookingId(Booking bookingInfo)
{
	...
	while(rs2.next()){
		BigInteger lastid =(BigInteger.valueOf(Long.parseLong(rs2.getString(1))));
